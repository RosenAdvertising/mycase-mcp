...
@mcp.tool()
def create_client(
    first_name: str, last_name: str, email: str = "", cell_phone_number: str = ""
) -> str:
    """Create a new client."""
    return json.dumps(
        MyCaseClient().create_client(
...
@mcp.tool()
def create_company(
    name: str, email: str = "", main_phone_number: str = "", website: str = ""
) -> str:
    """Create a new company."""
    return json.dumps(
        MyCaseClient().create_company(
            name=name,
            email=email or None,
            main_phone_number=main_phone_number or None,
...
def create_time_entry(
    ...
    return json.dumps(
        MyCaseClient().create_time_entry(
            case_id=case_id,
            staff_id=staff_id,
...
def record_invoice_payment(
    ...
    return json.dumps(
        MyCaseClient().record_invoice_payment(
            invoice_id=invoice_id, amount=amount, date=date, notes=notes or None
        ),
        indent=2,
    )
...
def update_note(note_id: int, note: str = "", subject: str = "", date: str = "") -> str:
    ...
    return json.dumps(
        MyCaseClient().update_note(
            note_id, note=note or None, subject=subject or None, date=date or None
        ),
        indent=2,
    )
...
def create_client_note(client_id: int, note: str, subject: str, date: str) -> str:
    ...
    return json.dumps(
        MyCaseClient().create_client_note(
            client_id, note=note, subject=subject, date=date
        ),
        indent=2,
    )
...
def create_company_note(company_id: int, note: str, subject: str, date: str) -> str:
    ...
    return json.dumps(
        MyCaseClient().create_company_note(
            company_id, note=note, subject=subject, date=date
        ),
        indent=2,
    )
...
def update_document(doc_id: int, name: str = "", description: str = "") -> str:
    ...
    return json.dumps(
        MyCaseClient().update_document(
            doc_id, name=name or None, description=description or None
        ),
        indent=2,
    )
...
def upload_document(
    ...
    return json.dumps(
        MyCaseClient().upload_document(
            ...
            path=path,
            description=description or None,
            assigned_date=assigned_date or None,
...
def upload_case_document(
    ...
    return json.dumps(
        MyCaseClient().upload_case_document(
            case_id=case_id,
            filename=filename,
            path=path,
            description=description or None,
...
def create_lead(
    ...
    email: str = "",
    cell_phone_number: str = "",
    referral_source_id: int = 0,
) -> str:
    """Create a new lead."""
    return json.dumps(
        MyCaseClient().create_lead(
            first_name=first_name,
            last_name=last_name,
            email=email or None,
            cell_phone_number=cell_phone_number or None,
...
def create_message_thread(
    ...
    cids = (
        [int(x.strip()) for x in client_ids.split(",") if x.strip()]
        if client_ids
        else None
    )
    sids = (
        [int(x.strip()) for x in staff_ids.split(",") if x.strip()]
        if staff_ids
        else None
    )
    return json.dumps(
        MyCaseClient().create_message_thread(
            subject=subject,
...
def create_case_message_thread(
    ...
    return json.dumps(
        MyCaseClient().create_case_message_thread(
            case_id=case_id,
            subject=subject,
            first_message_body=first_message_body,
            sender_id=sender_id or None,
...
def post_message(thread_id: int, body: str, sender_id: int = 0) -> str:
    ...
    return json.dumps(
        MyCaseClient().post_message(
            thread_id, body_text=body, sender_id=sender_id or None
        ),
        indent=2,
    )
...
def create_custom_field(
    name: str, parent_type: str, field_type: str, list_options: str = ""
) -> str:
    ...
    opts = (
        [v.strip() for v in list_options.split(",") if v.strip()]
        if list_options
        else None
    )
    return json.dumps(
        MyCaseClient().create_custom_field(name, parent_type, field_type, opts),
        indent=2,
    )
...
def update_call(
    ...
    return json.dumps(
        MyCaseClient().update_call(
            call_id,
            caller_name=caller_name or None,
            caller_phone_number=caller_phone_number or None,
            call_for=call_for or None,
            message=message or None,
            call_type=call_type or None,
...
def create_webhook_subscription(model: str, url: str, actions: str) -> str:
    ...
    return json.dumps(
        MyCaseClient().create_webhook_subscription(model, url, action_list), indent=2
    )
