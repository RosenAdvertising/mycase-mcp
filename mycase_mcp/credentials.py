#!/usr/bin/env python3
"""Pluggable credential storage for mycase-mcp.

Secrets (OAuth client ID, client secret) are stored in the operating system's
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.mycase-mcp/.env file
...
user opts out of keyring with ``MYCASE_MCP_USE_KEYRING=0``.
...
SERVICE_NAME = "mycase-mcp"
CONFIG_DIR = Path.home() / ".mycase-mcp"
ENV_FILE = CONFIG_DIR / ".env"
_USE_KEYRING_FLAG = "MYCASE_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``MYCASE_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
